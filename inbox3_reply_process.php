...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($reply_text == "end"){
        ...
        $conn->query($query);
        $conn->close();
        header("Location: manager_dashboard.php?success=Conversion ended");
        exit();
    }
    ...
    if ($conn->query($query) === FALSE) {
        ...
        header("Location: manager_dashboard.php?success=Reply message sent successfully.");
        exit();
    }
} else {
    // Redirect to the user profile page if the form is not submitted
    $conn->close();
    header("Location: manager_dashboard.php?error=An error occurred while sending the reply message");
    exit();
}
?>
